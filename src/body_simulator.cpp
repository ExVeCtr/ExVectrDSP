...
BodySimulator::BodySimulator(float mass, Math::Vector<float, 3> inertiaTensor,
  ...
  bodySim_.setInertiaTensor(
      inertiaTensor_);  // Set the inertia tensor of the body simulator
  ...
  tvcPosition_ = {
      0, 0, -0.35};  // Position of the thrust vector control in body frame

  tvcAngleLimit_ =
      tvcAngleLimit_Rad;  // Set the thrust vector control angle limit
  tvcForceLimit_ =
      tvcThrustLimit_N;  // Set the thrust vector control force limit
}
...
void BodySimulator::update() {
  ...
  if (tvcSubr_.isDataNew()) {
    ...
    Math::Vector<float, 3> tvcVector = {tvcInput(0), tvcInput(1),
                                        tvcInput(2)};  // Force vector
    ...
    auto tvcRotationAxis =
        (tvcVector.cross(Math::Vector<float, 3>({0, 0, 1})))
            .normalize();  // Rotation axis is the cross product of the vector
                           // and the Z-Axis.
    if (tvcAngle > tvcAngleLimit_) {
      auto tvcRotation =
          Math::Quat_F(tvcRotationAxis, tvcAngle - tvcAngleLimit_).conjugate();
      tvcVector = tvcRotation.rotate(
    ...
    if (tvcMagnitude > tvcForceLimit_) {
      tvcVector = tvcVector.normalize() *
                  tvcForceLimit_;  // Scale the vector back to the limit.
    }

    tvcInput(0) = tvcVector(0);
    tvcInput(1) = tvcVector(1);
    tvcInput(2) =
        tvcVector(2);  // Update the input vector with the limited vector.

    // LOG_MSG("TVC Force: %.2f %.2f %.2f |%.1f|\n", tvcVector(0), tvcVector(1),
    // tvcVector(2), tvcVector.magnitude()); // Print the force vector to the
    // console
    ...
    tvcZTorque_ =
        tvcInput(3) * 1.5;  // Get the torque around the Z axis from the thrust
                            // vector control input in body frame
    ...
    if (tvcEnabled_) {
      totalBodyForce =
          totalBodyForce +
          tvcForce;  // Add the force vector to the total force vector
      totalBodyTorque =
          totalBodyTorque +
          tvcTorque;  // Add the torque vector to the total torque vector
    }

    // tvcAngle = tvcVector.getAngleTo(Math::Vector<float, 3>({0, 0, 1})); //
    // Get the angle of the force vector to the Z-axis

    // LOG_MSG("TVC Force: %.2f %.2f %.2f |%.1f|, Angle: %.2f\n",
    // totalBodyForce(0), totalBodyForce(1), totalBodyForce(2),
    // totalBodyForce.magnitude(), tvcAngle/DEGREES); // Print the force vector
    // and torque to the console
  }

  if (Core::NOW() - lastWindUpdateTime_ > 1 * Core::SECONDS) {
    // Update wind velocity every 100 ms
    lastWindUpdateTime_ = Core::NOW();
    // Simple wind model: random gusts

    const float maxWindSpeed = 5.0f;  // m/s
    const float minWindSpeed = 0.0f;  // m/s
    const Math::Vector<float, 3> windDirection = {1, 0, 0};
    const Math::Vector<float, 3> windBias = {1, 1, 0};
  ...
  auto attitude =
      bodySim_.getAttitude();  // Get the attitude from the body simulator
  auto angularVelocity =
      bodySim_.getAngularVelocity();  // Get the angular velocity from the body
                                      // simulator
  auto velocity =
      bodySim_.getVelocity();  // Get the velocity from the body simulator
  windVelocity = 0;
  auto velocityBody = attitude.rotate(velocity) + windVelocity;

  // LOG_MSG("AngVel: %.2f %.2f %.2f |%.1f|\n", angularVelocity(0)/DEGREES,
  // angularVelocity(1)/DEGREES, angularVelocity(2)/DEGREES,
  // angularVelocity.magnitude()/DEGREES); // Print the angular velocity to the
  // console
  ...
  bodySim_.addTorque({0, 0, tvcZTorque_},
                     0);  // Add the tvc twist torque in body frame

  if (velocity(2) < -1 || true) {  // We are moving down. Start simulating the
                                   // pseudo drag force for the belly flop.

    const float lambda = 0.0436;  // drag pseudo coefficient
    auto velMag =
        velocity.magnitude();  // Get the magnitude of the velocity vector
    auto dragForce =
        (-velocity) *
        (lambda * velMag);  // Calculate the drag force in body frame

    // dragForce = attitude.rotate(dragForce); // Rotate the drag force to the
    // body frame LOG_MSG("Drag force: %.2f %.2f %.2f |%.1f|\n", dragForce(0),
    // dragForce(1), dragForce(2), dragForce.magnitude()); // Print the drag
    // force to the console bodySim_.addForce(dragForce, {0.02, 0, 0});

    auto cylBodyForce = calcCylForce(velocityBody);
    auto cylTopForce =
        calcTopBottomForce(velocityBody, Math::Vector_F({0, 0, 1}));
    auto cylBottomForce =
        calcTopBottomForce(velocityBody, Math::Vector_F({0, 0, -1}));

    // LOG_MSG("Vel: %.2f %.2f %.2f |%.1f|\n", velocityBody(0), velocityBody(1),
    // velocityBody(2), velocityBody.magnitude()); LOG_MSG("Force: %.2f %.2f
    // %.2f |%.1f|\n", cylBottomForce(0), cylBottomForce(1), cylBottomForce(2),
    // cylBottomForce.magnitude());
    ...
    //         velocityBody(1), velocityBody(2), velocityBody.magnitude());
    ...
    //         cylBodyForce(0), cylBodyForce(1), cylBodyForce(2),
    //         cylBodyForce.magnitude(), cylTopForce(0), cylTopForce(1),
    //         cylTopForce(2), cylTopForce.magnitude(), cylBottomForce(0),
    //         cylBottomForce(1), cylBottomForce(2),
    //         cylBottomForce.magnitude());
  ...
  if (zeroingMode_) {
    ...
    attitudeState_ = {0, 0, 0, 0,
                      0, 0, 1};   // Set the attitude to the reference frame
    ...
    bodySim_.setPositionState(
        positionState_);  // Set the position state to the body simulator
    bodySim_.setAttitudeState(
        attitudeState_);  // Set the attitude state to the body simulator
  }
  // bodySim_.setAttitudeState({0, 0, 0, 0, 0, 0, 1});

  if (positionState_(5) < 0) {
    if (positionState_(2) < 0)  // Only limit the velocity to 0 if the position
                                // is below 0. We want to be able to go back up.
      positionState_(2) = 0;
    positionState_(0) = 0;
    positionState_(1) = 0;
    positionState_(5) = 0;
...
void BodySimulator::taskThread() {
  ...
  groundState.data = {
      positionState_(2),
      positionState_(
          5)};  // Formed as: [V, P], where V is the vertical velocity in m/s
                // and P is the ground distance in meters
...
void BodySimulator::calcFlapForces(const Math::Vector_F& velocity) {
  ...
  auto tlRot = Math::Quat<float>(Math::Vector_F({0, 0, 1}),
                                 flapSettings.flapTLAngle_Rad);
  auto trRot = Math::Quat<float>(Math::Vector_F({0, 0, 1}),
                                 -flapSettings.flapTRAngle_Rad);
  auto blRot = Math::Quat<float>(Math::Vector_F({0, 0, 1}),
                                 flapSettings.flapBLAngle_Rad);
  auto brRot = Math::Quat<float>(Math::Vector_F({0, 0, 1}),
...
Math::Vector_F BodySimulator::calcTopBottomForce(
    const Math::Vector_F& velocityBody, const Math::Vector_F& normal) {
  const float cylFlatArea =
      2 * M_PI * (0.12 / 2) * (0.12 / 2);  // Top bottom surfaces
  const float flatCoeff = 1.17;

  auto angle = velocityBody.getAngleTo(normal);
  if (angle > 90 * DEGREES) return 0;

  auto velProj =
      velocityBody.getProjectionOn(normal, false);  // * 0.7 + velocityBody*0.3;
  auto velProjDrag = velocityBody.getProjectionOn(normal, true);

  return -velProj *
             (0.5 * 1.2 * flatCoeff * cylFlatArea * velProj.magnitude()) -
         cos(angle) * velProjDrag *
             (0.3 * 0.5 * 1.2 * flatCoeff * cylFlatArea *
              velProjDrag.magnitude());
}

Math::Vector<float, 3> BodySimulator::calcForceFromTVC(
    const Math::Vector<float, 4>& tvcInput) {
  return {tvcInput(0), tvcInput(1), tvcInput(2)};  // Force vector in body frame
}

Math::Vector<float, 3> BodySimulator::calcTorqueFromTVC(
    const Math::Vector<float, 4>& tvcInput,
    const Math::Vector<float, 3>& tvcPosition) {
  Math::Vector<float, 3> forceVector = {
      tvcInput(0), tvcInput(1), tvcInput(2)};  // Force vector in body frame
  Math::Vector<float, 3> torqueVector =
      forceVector.cross(tvcPosition);  // Torque vector in body frame
  Math::Vector<float, 3> rollTorque = {
      0, 0, tvcInput(3)};  // Roll torque in body frame.
