...
void IMUAttitudeCF::update() {
  ...
  if (update) {  // Fix covariance and publish the new data.
    ...
    // attitudeData.data.cov.block(p_, 3, 3, 3, 3); //Copy the lower right part
    // from p for attitude quat into cov mat.
    attitudeData.data.block(lastGyroData_.data.val - x_.block<3, 1>(0, 0), 0, 0,
                            0, 0, 3, 1);
    // attitudeData.data.cov.block(lastGyroData_.data.cov * 2, 0, 0, 0, 0, 3,
    // 3);
    ...
    // biasData.data.cov.block(p_, 0, 0, 0, 0, 3, 3); //Copy the lower right
    // part from p for attitude quat into cov mat.
...
void IMUAttitudeCF::updateGyro(
  ...
  // x_[3][0] = quat[0][0] - dTimeHalf * (gyro[0][0] * quat[1][0] + gyro[1][0] *
  // quat[2][0] + gyro[2][0] * quat[3][0]); x_[4][0] = quat[1][0] + dTimeHalf *
  // (gyro[0][0] * quat[0][0] - gyro[1][0] * quat[3][0] + gyro[2][0] *
  // quat[2][0]);
  /// x_[5][0] = quat[2][0] + dTimeHalf * (gyro[0][0] * quat[3][0] + gyro[1][0]
  /// * quat[0][0] - gyro[2][0] * quat[1][0]);
  // x_[6][0] = quat[3][0] - dTimeHalf * (gyro[0][0] * quat[2][0] - gyro[1][0] *
  // quat[1][0] - gyro[2][0] * quat[0][0]);
  ...
  auto P =
      F * p_.block<4, 4>(3, 3) * F.transpose() + W * gyroCov * W.transpose();
  p_.block(P, 3, 3);

  lastGyroData_ = gyroData;
}

void IMUAttitudeCF::updateAcc(
  ...
  auto accErrorIndex =
      (quat.conjugate().rotate(accData.data.val) - Math::GRAVITY_3F)
          .magnitude();
  auto accCov = accData.data.cov *
                (5000 * (accErrorIndex / 10 +
                         1));  // Increase the covariance if the accel is far
  ...
  auto h = Math::Matrix<float, 3, 1>(
      {2 * (quat[1][0] * quat[3][0] - quat[0][0] * quat[2][0]),
       2 * (quat[0][0] * quat[1][0] + quat[2][0] * quat[3][0]),
       2 * (0.5f - quat[1][0] * quat[1][0] - quat[2][0] * quat[2][0])});

  // Jacobian of measurement model
  auto H = VCTR::Math::Matrix<float, 3, 4>(
      {-2 * quat[2][0], 2 * quat[3][0], -2 * quat[0][0], 2 * quat[1][0],
       2 * quat[1][0], 2 * quat[0][0], 2 * quat[3][0], 2 * quat[2][0], 0,
       -4 * quat[1][0], -4 * quat[2][0], 0});
  ...
  auto x = quat + K * y;
  auto P =
      (VCTR::Math::Matrix<float, 4, 4>::eye() - K * H) * p_.block<4, 4>(3, 3);
  ...
  auto gyroModelRot =
      -lastAccData_.data.val.cross(acc).normalize() *
      (lastAccData_.data.val.getAngleTo(Math::Vector_F(acc))) /
      (float(accData.timestamp - lastAccData_.timestamp) / Core::SECONDS);
...
void IMUAttitudeCF::updateMag(
  ...
  // auto magRef = VCTR::Math::Matrix<float, 3, 1>({1, 0, 0}); // Reference
  // magnetic field vector

  // Calculate the rotation vector between the reference and observed magnetic
  // field vectors
  ...
  /// ############ below is the standard EKF but not currently use as it has
  /// issues ############

  /*
  // Magnetic Measurement model
  auto h = VCTR::Math::Matrix<float, 3, 1>({2 * (0.5f - quat[2][0] * quat[2][0]
  - quat[3][0] * quat[3][0]), 2 * (quat[1][0] * quat[2][0] - quat[0][0] *
  quat[3][0]), 2 * (quat[0][0] * quat[2][0] + quat[1][0] * quat[3][0])});

  // Jacobian of measurement model
  auto H = VCTR::Math::Matrix<float, 3, 4>({0, 0, -4 * quat[2][0], -4 *
  quat[3][0], -2 * quat[3][0], 2 * quat[2][0], 2 * quat[1][0], -2 * quat[0][0],
                                            2 * quat[2][0], 2 * quat[3][0], 2 *
  quat[0][0], 2 * quat[1][0]});
  ...
  // Update state and covariance
  auto x = quat + K * y;
  auto P = (VCTR::Math::Matrix<float, 4, 4>::eye() - K * H) * p_.block<4, 4>(3,
  3);

  // Normalize quaternion and update matricies
  float norm = sqrt(x[0][0] * x[0][0] + x[1][0] * x[1][0] + x[2][0] * x[2][0] +
  x[3][0] * x[3][0]); if (x[0][0] < 0) // Normalize so the w component is always
  positive
  {
      norm = -norm;
  }
  ...
  // Update gyroBias using a simple low pass filter
  auto gyroBias = x_.block<3, 1>(0, 0);
  auto gyroModelRot = lastMagData_.data.val.cross(mag) /
  (float(magData.timestamp - lastMagData_.timestamp)/Core::SECONDS); gyroBias =
  gyroBias * 0.999 + (lastGyroData_.data.val - gyroModelRot) * 0.001;
...
void IMUAttitudeCF::initialiseAcc(
    const VCTR::Core::Timestamped<VCTR::DSP::ValueCov<float, 3U>>& accData) {
  Math::Quat<float> quat = x_.block<4, 1>(3, 0);
...
void IMUAttitudeCF::initialiseMag(
    const VCTR::Core::Timestamped<VCTR::DSP::ValueCov<float, 3U>>& magData) {
  Math::Quat<float> quat = x_.block<4, 1>(3, 0);
  ...
  mag(2) = 0;
  mag = quat.rotate(mag.normalize());  // Normalize and rotate back to body
                                       // frame
...
void IMUAttitudeCF::setProcessNoise(
    const VCTR::Math::Matrix<float, 7, 7>& noise) {
  // q_ = noise;
}

const VCTR::Math::Vector<float, 7>& IMUAttitudeCF::getState() { return x_; }
...
IMUAttitudeCF::getBiasEstTopic() {
  return biasTopic_;
}

Core::Topic<Core::Timestamped<Math::Vector<float, 7>>>&
IMUAttitudeCF::getAttitudeEstTopic() {
...
IMUAttitudeCFTask::IMUAttitudeCFTask(int64_t period, Core::Scheduler& scheduler)
    : Task_Periodic("IMUAttitudeCFTask", period) {
...
void IMUAttitudeCFTask::taskThread() {
  // if (!gyroSubr_.isDataNew())
  //{
  //     //predict(); //Predict up to this point to keep the state up to date
  //     for corrections
  // }
