...
namespace VCTR
{
    ...
    namespace DSP
    {
        ...
        protected:
            ...
            Core::Timestamped<SNSR::GNSSData> lastGNSSData_;
            ...
            Math::Vector<float, 3> accelBias_ = 0; // An additional bias value generated by the filter to correct the accelerometer data. This is in body frame.
            Math::Vector<double, 3> gnssRefPos_ = 0;

            /// @brief When enabled, then the filter attempts to zero the position and velocity, the reference position is set to the current position and finally the barometer and accelerometer are calibrated assuming no motion.
            bool zeroingMode_ = false;
            /// @brief When enabled, the filter will use the range finders ground estimate for the altitude.
            ...
            Core::Topic<Core::Timestamped<VCTR::Math::Vector<float, 2>>> groundDistanceEstTopic_;
            Core::Topic<Core::Timestamped<VCTR::Math::Matrix<float, 2, 2>>> groundDistanceCovTopic_;
        ...
        public:
             ...
             */
            void setAttitudeInput(Core::Topic<Core::Timestamped<Math::Vector<float, 7>>> &attitudeTopic);
            ...
            /**
             * @brief Sets the range input topic. The range is assumed to be the distance to the ground in body frame.
             * @note The range input should be in [m]. The Covariance in meters.
             * @param rangeTopic Topic for range input.
             * @param minRange Minimum range in meters. If the range is below this value, it is ignored.
             * @param maxRange Maximum range in meters. If the range is above this value, it is ignored.
             */
            void setRangeInput(Core::Topic<Core::Timestamped<ValueCov<float, 1>>> &rangeTopic, float minRange = 0, float maxRange = -1);
             ...
             * @brief When enabled, the filter will use the range finders ground estimate for the altitude.
             */
            void setUseRangeForAltitude(bool useRangeForAltitude = true) { useRangeForAltitude_ = useRangeForAltitude; }

            /**
             * @brief Sets the reference position for the GNSS data. This is used to calculate the relative position from the GNSS data.
             * @param refPos Reference position for the GNSS data in form [Latitude, Longitude, Altitude].
             */
            const Math::Vector<double, 3> &getPositionReference()
            {
                return gnssRefPos_;
            }
             ...
             * @param attitudeData
             */
            // void updateAttitude(const Core::Timestamped<Math::Vector<float, 7>> & attitudeData);
             ...
             * @param rangeData
             */
            void updateRange(const VCTR::Core::Timestamped<VCTR::DSP::ValueCov<float, 1U>> &rangeData);
             ...
             * @param gnssData
             */
            void updateAccBias(const VCTR::Core::Timestamped<VCTR::SNSR::GNSSData> &gnssData, float assumeZero = false);
            ...
            /**
             * @brief Initialises the ground distance estimation using an observation of the range.
             * @param rangeData
             */
            void initialiseRange(const VCTR::Core::Timestamped<VCTR::DSP::ValueCov<float, 1U>> &rangeData);
            ...
            /**
             * @note The ground distance estimation is the estimated distance to the ground.
            ...
            /**
             * @note The ground distance covariance is the estimated covariance of the ground distance.
             ...
             */
            Core::Topic<Core::Timestamped<VCTR::Math::Vector<float, 2>>> &getGroundDistanceEstTopic();

            /**
             * @return The state estimation covarience for velocity and position in form: [V, P].
             */
             ...
             */
            Core::Topic<Core::Timestamped<VCTR::Math::Matrix<float, 2, 2>>> &getGroundDistanceCovTopic();
